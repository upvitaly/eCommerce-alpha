                ...
                <table id="datatable1" class="table display responsive nowrap">
                    <thead>
                        <tr>
                            ...
                            <th class="wd-15p">Post Category En</th>
                            <th class="wd-15p">Post Category In</th>
                            <th class="wd-20p">Action</th>

                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($postcat as $key => $row)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $row->category_name_en }}</td>
                                <td>{{ $row->category_name_in }}</td>
                                <td>
                                    <a href="{{ URL::to('/edit/post/category/' . $row->id) }} "
                                        class="btn btn-sm btn-info">Edit</a>
                                    <a href="{{ URL::to('/delete/post/category/' . $row->id) }}"
                ...
                <form method="POST" action="{{ route('post.category.store') }}">
                    <div class="modal-body pd-20">
                        ...
                        <div class="form-group">
                            <label for="catname">PostCategory English</label>
                            <input id="catname" type="text" class="form-control" name="category_name_en"
                                placeholder="Input category here">
                        </div>
                        <div class="form-group">
                            <label for="catname">PostCategory Hindi</label>
                            <input id="catname" type="text" class="form-control" name="category_name_in"
                                placeholder="Input category here">
                        </div>
                    </div><!-- modal-body -->
